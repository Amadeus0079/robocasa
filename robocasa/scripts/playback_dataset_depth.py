...
def depth_to_heatmap(
    ...
    """
    Convert single-channel depth to 3-channel heatmap (BGR by default).
    Args:
        depth: (H,W) or (H,W,1) array-like, may contain NaN/inf.
        colormap: OpenCV colormap (e.g. cv2.COLORMAP_JET).
        vmin, vmax: optional fixed min/max for normalization. If None use finite values.
        eps: small value to avoid division by zero.
        to_bgr: if True return BGR (cv2 default). If False return RGB.
        resize_to: optional (H,W) tuple to resize output to.
    Returns:
        heatmap: uint8 array shape (H,W,3).
    """
    ...
    if vmin is None or vmax is None:
        if finite_mask.any():
            auto_min = float(np.nanmin(d[finite_mask]))
            auto_max = float(np.nanmax(d[finite_mask]))
        else:
    ...
    heat = cv2.applyColorMap(norm, colormap)  # returns BGR uint8

    if not to_bgr:
        heat = cv2.cvtColor(heat, cv2.COLOR_BGR2RGB)

    if resize_to is not None and heat.shape[:2] != resize_to:
        heat = cv2.resize(heat, (resize_to[1], resize_to[0]), interpolation=cv2.INTER_LINEAR)
...
def playback_trajectory_with_obs(
    ...
    for i in range(traj_len):
        if video_count % video_skip == 0:
            ...
            # concatenate depth obs together
            depth = [traj_grp["obs/{}".format(k + "_depth")][i] for k in image_names]
            depth_frame = np.concatenate(depth, axis=1)
            depth_frame = depth_to_heatmap(depth_frame, to_bgr=False)
            
            # stack rgb and depth vertically
            combined_frame = np.vstack((frame, depth_frame))
